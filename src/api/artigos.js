...
router.post('/', async (req, res) => {
  try {
    const { autor_id, titulo, conteudo, tags, visualizacoes } = req.body;

    // Verificar se o autor existe
    const autorQuery = 'SELECT * FROM facens.autores WHERE autor_id = ?';
    const autorResult = await client.execute(autorQuery, [autor_id], { prepare: true });
    ...
    const artigoQuery = 'INSERT INTO facens.artigos (autor_id, id, titulo, conteudo, data_publicacao, tags, visualizacoes) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const params = [autor_id, id, titulo, conteudo, data_publicacao, tags, visualizacoes];
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updateQuery = 'UPDATE facens.artigos SET titulo = ?, conteudo = ?, tags = ? WHERE id = ?';
    const params = [titulo, conteudo, tags, id];
